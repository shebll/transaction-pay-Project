package services;
import java.util.*;
 

public class Services {
	private Map<String,Vector<String>> mp = new HashMap<String, Vector<String>>();
	public Services(){
	Vector<String>vec=new Vector<String>();
	vec.add("Vodafone");
	vec.add("Etisalat");
	vec.add("Orange");
	vec.add("We");
	Vector<String>vec1=new Vector<String>();
	vec1.add("Monthly_receipt");
	vec1.add("Quarter_receipt");
	Vector<String>vec2=new Vector<String>();
	vec2.add("Cancer_Hospital");
	vec2.add("Schools");
	vec2.add("NGOs");
	mp.put("Landline", vec1);
	mp.put("Mobile_recharge", vec);
	mp.put("Internet_Payment", vec);
	mp.put("Donations", vec2);
	}	
	public void searchSrevices(String name){
		if(mp.get(name) != null){
			System.out.print(name+"\n");
			System.out.print(mp.get(name)+"\n\n");	
		}
		else {
			System.out.print("this services not fount."+"\n");
		}	 
	}

	public Map<String,Vector<String>> getMp() {
		return mp;
	}

	public void setMp(Map<String,Vector<String>> mp) {
		this.mp = mp;
	}
	
	
	
	
}
